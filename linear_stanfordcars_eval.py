...
def main(args,class_num):
    ...
    classifier = nn.Linear(in_features=2048, out_features=classes, bias=True).to(args.device)
    ...
    for epoch in global_progress:
        ...
        for idx, (images, labels) in enumerate(local_progress):
            ...
            with torch.no_grad():
                feature, bp_out_feat = model.module.inference(images.to(args.device))

            preds = classifier(bp_out_feat)
    ...
    for idx, (images, labels) in enumerate(test_loader):
        with torch.no_grad():
            ...
            preds = classifier(bp_out_feat).argmax(dim=1)
